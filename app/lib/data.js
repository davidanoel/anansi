...
import { PACKAGE_ID, REGISTRY_ID, LOT_STATUS } from './constants'
...
export async function getRecentDeliveries(limit = 20) {
  const events = await queryEvents(
    `${PACKAGE_ID}::asset_pool::DeliveryRecorded`,
    null,
    limit
  )
...
export async function getTokenPortfolio(address) {
  const client = getSuiClient()
  const type = `${PACKAGE_ID}::asset_pool::SpiceToken`
...
export async function getRegistryAdmin(address) {
  const type = `${PACKAGE_ID}::asset_pool::RegistryAdmin`
...
export async function getCustodianCaps(address) {
  const type = `${PACKAGE_ID}::asset_pool::CustodianCap`
  const result = await getOwnedObjects(address, type)
...
export async function getAssetTypes() {
  const events = await queryEvents(
    `${PACKAGE_ID}::asset_pool::AssetTypeCreated`,
    null,
    50
  )
...
export async function getBurnHistory(limit = 50) {
  const events = await queryEvents(
    `${PACKAGE_ID}::carib_coin::TokensBurned`,
    null,
    limit
  )
