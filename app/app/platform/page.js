    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-anansi-gray">Register and manage tokenizable Caribbean assets.</p>
        </div>
        <button
              ...
              {result.success ? (
                <div>
                  ...
                  {result.data?.objects?.map((obj, i) => (
                    <p key={i} className="font-mono text-xs mt-1">{obj.type}: {obj.id}</p>
                  ))}
                </div>
              ) : `Error: ${result.error}`}
            </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border border-anansi-border rounded-xl bg-white">
          ...
          <p className="text-anansi-gray text-sm mt-1">Click "+ New Asset Type" to register your first Caribbean asset.</p>
        </div>
      ) : (
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-anansi-gray">Manage who can create lots and record deliveries.</p>
        </div>
        <button
          onClick={() => setShowForm(!showForm)}
          className="px-4 py-2 bg-anansi-black text-white rounded-lg text-sm hover:bg-anansi-red transition-colors"
      ...
      {showForm && (
        <div className="p-6 border-2 border-anansi-red/30 bg-white rounded-xl">
          ...
          <form onSubmit={handleIssue} className="space-y-4">
            <div>
              ...
              >
                ...
                {assetTypes.map(at => (
                  <option key={at.id} value={at.id}>{at.symbol} — {at.name}</option>
                ))}
              </select>
            </div>
            <Field
              ...
              required
            />
            <button type="submit" disabled={submitting || !form.assetTypeId || !form.custodianAddress}
              className="px-6 py-2.5 bg-anansi-red text-white rounded-lg text-sm font-medium disabled:opacity-50 hover:bg-red-700 transition-colors">
          ...
          {result && (
            <div className={`mt-4 p-3 rounded-lg text-sm ${result.success ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
              {result.success ? `Custodian cap issued. Tx: ${result.digest}` : `Error: ${result.error}`}
            </div>
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border border-anansi-border rounded-xl bg-white">
          ...
          <p className="text-anansi-gray text-sm mt-1">Custodians are created automatically when you register an asset type, or you can issue additional caps above.</p>
        </div>
      ) : (
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 border border-anansi-border rounded-xl bg-white">
        ...
        <div className="space-y-2 text-sm font-mono">
          ...
          <EnvRow label="Package" value={process.env.NEXT_PUBLIC_PACKAGE_ID?.slice(0, 16) + '...'} />
...
function StatCard({ label, value }) {
  return (
    <div className="p-4 border border-anansi-border rounded-xl bg-white">
      ...
      <p className="text-lg font-bold mt-1">{typeof value === 'string' ? value : String(value ?? '—')}</p>
    </div>
  )
}
...
function RoleRow({ role, path, description, color }) {
  return (
    <div className="flex gap-3">
      ...
      <div>
        <p className="font-semibold">{role} <span className="font-mono text-xs text-anansi-gray">{path}</span></p>
