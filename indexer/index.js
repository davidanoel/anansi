...
import { config, getEventTypes } from './config.js'
...
function processEvent(event, txDigest, timestamp) {
  ...
  try {
    if (type === eventTypes.LOT_CREATED) {
    ...
    else if (type === eventTypes.TOKENS_TRANSFERRED) {
      db.updateTokenBalance(data.from, data.lot_id, -Number(data.amount))
      db.updateTokenBalance(data.to, data.lot_id, Number(data.amount))
      console.log(`[TRANSFER] ${data.amount} tokens ${data.from.slice(0, 8)}... → ${data.to.slice(0, 8)}...`)
    }
      ...
      console.log(`[SURPLUS] $${Number(data.net_amount) / 1e6} for lot ${data.lot_id.slice(0, 8)}...`)
    }
      ...
      db.insertSurplusClaim({
        ...
        tokens_redeemed: Number(data.tokens_redeemed),
        usdc_received: Number(data.usdc_received),
        tx_digest: txDigest,
        timestamp,
      })
      console.log(`[CLAIM] ${data.claimant.slice(0, 8)}... claimed $${Number(data.usdc_received) / 1e6}`)
    }
...
async function pollEvents() {
  ...
  for (const eventType of allTypes) {
    ...
    try {
      ...
      // Don't crash on individual event type failures
...
app.get('/api/carib/burns', (req, res) => {
  const burns = db.getDb().prepare('SELECT * FROM carib_burns ORDER BY timestamp DESC LIMIT 100').all()
  res.json({ burns, totalBurned: db.getTotalBurned() })
})

// Fee collection history
app.get('/api/carib/fees', (req, res) => {
  const fees = db.getDb().prepare('SELECT * FROM fee_collections ORDER BY timestamp DESC LIMIT 100').all()
  res.json(fees)
})
