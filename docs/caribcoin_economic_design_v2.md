...
- **Staking friction** — while the protocol does not lock tokens, staked tokens must go through a 24-hour cooldown to unstake, dampening flash-sell behavior
...
Staking CARIB is voluntary and flexible. There are no fixed lock periods and no slashing. Stakers receive protocol-level benefits while their tokens are staked and active.

| Benefit | Requirement | How it works |
|---|---|---|
| Reduced platform fees | Stake ≥ 10,000 CARIB | Up to 50% reduction on all Spice and CaribStone fees |
...
- Distribute revenue or dividends from the protocol
- Promise returns of any kind
- Create an obligation from Anansi or the Foundation
- Lock tokens for a fixed duration
...
- **No fixed lock periods.** Stake any amount, any time. Benefits activate immediately.
- **Unstake at will with a 24-hour cooldown.** When a user requests unstake, their tokens enter a 24-hour cooldown. During cooldown: benefits deactivate (no voting, no fee discounts, no priority access), tokens cannot be moved or transferred.
- **Cancel cooldown at any time.** If the user changes their mind, they can cancel the pending unstake and restake instantly. No penalty. Benefits reactivate.
- **No slashing.** After the 24-hour cooldown, users withdraw their full balance. No fees, no penalties.
- **Ecosystem rewards** received while staking can be compounded — accepted directly into the stake position.

### 5.4 Rationale for the 24-Hour Cooldown

The cooldown exists for one reason: **to prevent flash-loan attacks on governance and priority access.** Without friction, an attacker could borrow millions of CARIB in a single transaction, stake, vote or claim priority, and unstake in the same block. A 24-hour cooldown makes this economically infeasible while still respecting the user's freedom to exit.

24 hours is the minimum friction required for the benefits to carry meaningful signal. Longer cooldowns would feel punitive without commensurate safety gain. Shorter cooldowns (or none) open the system to gaming.

The cooldown period is governable and can be adjusted by the Foundation within a bounded range (12–72 hours) if attack patterns emerge post-launch.

### 5.5 Staking as Supply Sink

Staking reduces circulating supply temporarily. Unlike burns, staked tokens are not destroyed — but while staked, they are not available for sale on the open market.

If 20% of circulating supply is staked at any given time, that's an additional supply reduction on top of burns. Combined effect: burns permanently reduce total supply, staking temporarily reduces circulating supply. Both create scarcity. Both are driven by real utility, not artificial lockups.

### 5.6 Regulatory Posture

The protocol does not pay yield to stakers. Benefits are access and governance weight — not returns, not distributions, not claims on revenue. This matters for regulatory positioning: a token that pays yield from protocol revenue may be classified as a security under US law (see Howey Test). A token whose staking gives access to product features is a utility.

Any yield opportunities in the CaribCoin ecosystem come from **external market activity**, not from the protocol. See Section 5.7.

### 5.7 Ecosystem Yield (External to Protocol)

While the CaribCoin protocol itself does not pay yield, the ecosystem around CaribCoin generates real economic activity. Participants can earn yield from three market-based mechanisms. **None of these are promises from the protocol:**

**1. DEX Liquidity Fees (Cetus CARIB/USDC pool)**
Liquidity providers on Cetus earn a share of every swap fee paid by traders. This is standard DEX economics — CaribCoin does not pay these fees; traders do. As fee events drive CARIB demand through auto-conversion, LP fee revenue grows proportionally with ecosystem activity.

**2. Foundation-Funded Liquidity Incentives**
The Foundation may run liquidity mining programs distributing CARIB from the Community allocation to LPs. These are treasury-discretion incentives, not embedded protocol rewards. They can be paused, adjusted, or ended based on strategic priorities.

**3. Third-Party Staking Services**
Validators, DeFi protocols, and staking-as-a-service providers may build products on top of CARIB that offer yield through their own arrangements. These are separate agreements between users and third parties — not CaribCoin itself.

This separation is deliberate. It preserves the protocol's regulatory posture while enabling real economic value for participants. Yield exists — but it is earned from market activity, not issued from protocol supply.
...
3. **The protocol never pays yield.** Staking benefits are fee reductions and access — earned by activity, not by holding. Yield opportunities exist only in the ecosystem (DEX fees, Foundation incentive programs, third-party services) and are never promises from the protocol itself.
4. **Unstaking is always permitted.** The only friction is a 24-hour cooldown to prevent governance attacks. No slashing. No penalties.
5. **Fees are always a percentage, never a flat tax that scales badly.** (Exception: pool creation, which is a small fixed amount to prevent spam.)
6. **USDC is always accepted.** CaribCoin is never the only payment method. Coercion kills adoption.
7. **Auto-conversion is transparent.** Every fee conversion is visible as a Sui event. Anyone can audit the burn rate, treasury inflows, and fee volumes in real time.
8. **The charter governs.** No economic design change can contradict the CaribCoin Charter's core principles: no promises, voluntary participation, free markets.
...
**Staking:** Flexible staking with no fixed lock periods. Unstake any time with a 24-hour cooldown. Provides fee reductions, governance weight, and priority access — but the protocol never pays yield. Creates a temporary supply sink without artificial lockups.

**Ecosystem Yield:** Real yield opportunities exist for participants through DEX liquidity provision (Cetus swap fees), Foundation-funded liquidity incentive programs, and third-party staking services. These are market-based, not protocol promises — which preserves regulatory posture while enabling real economic value.
