...
export const metadata = {
  ...
  description: 'Tokenize commodities, real estate, and revenue streams. Starting with Caribbean agriculture, scaling globally.',
}
    ...
    <>
      ...
      <section className="pt-40 pb-20 max-w-[1200px] mx-auto px-6 md:px-12">
        <div className="reveal">
          <p className="text-[10px] tracking-[0.25em] uppercase text-anansi-red font-medium mb-7">Spice — by Anansi</p>
      ...
      <section className="bg-anansi-white text-anansi-black border-y border-anansi-line-light">
        <div className="max-w-[1200px] mx-auto px-6 md:px-12 py-24">
          <div className="reveal">
            <p className="text-[10px] tracking-[0.25em] uppercase text-anansi-red font-medium mb-7">How It Works</p>
          ...
          <div className="space-y-0 reveal">
            {[
              { num: '01', title: 'James delivers nutmeg to GCNA', body: 'Same process as always. GCNA weighs, inspects, and pays the EC$ advance. Nothing changes for James here.' },
              { num: '02', title: 'GCNA records the delivery on Spice', body: 'The receiving officer enters the delivery into Spice. James receives NUTMEG tokens on his phone — each representing 1 kg of nutmeg in the pool.' },
              { num: '03', title: 'James has options he never had before', body: 'He can hold his NUTMEG and wait for the surplus (transparent, trackable). Or he can sell some NUTMEG immediately for USDC if he needs cash now. Instant liquidity.' },
              { num: '04', title: 'GCNA sells the nutmeg overseas', body: 'When the pool sells, surplus is automatically distributed to everyone holding NUTMEG. James gets his share deposited directly — no waiting months, no opacity.' },
            ].map((step, i) => (
              <div key={i} className="flex gap-6 items-baseline py-8 border-b border-anansi-line-light first:border-t">
                <span className="font-display font-bold text-2xl text-anansi-red/20 shrink-0">{step.num}</span>
                <div>
      ...
      <section className="py-24 max-w-[1200px] mx-auto px-6 md:px-12">
        <div className="reveal">
          <p className="text-[10px] tracking-[0.25em] uppercase text-anansi-red font-medium mb-7">Beyond Nutmeg</p>
          ...
          <p className="text-anansi-gray max-w-[540px] mb-12">
            The same system that tokenizes nutmeg can tokenize any real-world
            asset. Adding a new commodity, a new country, or a new asset class
        ...
        <div className="grid md:grid-cols-3 gap-px bg-anansi-line reveal-stagger">
          {[
            { phase: 'Now', title: 'Agriculture', items: 'Grenada Nutmeg · Jamaica Coffee · Grenada Cocoa · Mace · Spices' },
            { phase: 'Next', title: 'Real Estate', items: 'Caribbean Villas · Boutique Hotels · Fractional Ownership · Diaspora Investment' },
            { phase: 'Future', title: 'Revenue Streams', items: 'Rum Distillery Cash Flows · Tourism Revenue · Carbon Credits · Renewable Energy' },
          ].map((card, i) => (
            <div key={i} className="bg-anansi-deep p-10">
              <p className="text-[9px] tracking-[0.25em] uppercase text-anansi-red mb-5 font-medium">{card.phase}</p>
              <h3 className="font-display font-bold text-[22px]">{card.title}</h3>
              <p className="text-[14px] text-anansi-gray mt-4 leading-[2.1]">{card.items.split(' · ').join('\n')}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-anansi-deep border-t border-anansi-line">
        <div className="max-w-[1200px] mx-auto px-6 md:px-12 py-24">
          <div className="reveal">
            <p className="text-[10px] tracking-[0.25em] uppercase text-anansi-red font-medium mb-7">Technology</p>
            <h2 className="font-display font-bold text-3xl mb-12">Zero friction by design.</h2>
          </div>

          <div className="grid md:grid-cols-3 gap-px bg-anansi-line reveal-stagger">
            {[
              { title: 'Sign in with Google', body: 'Sui zkLogin creates an invisible blockchain wallet from your Google account. No seed phrases. No wallet apps. No crypto knowledge required.' },
              { title: 'Zero gas fees', body: 'Anansi sponsors every transaction. Users never pay fees, never need SUI tokens, never see a gas prompt. The blockchain is invisible.' },
              { title: 'Sub-second finality', body: 'Built on Sui — transactions confirm in under a second with fees under $0.003. Fast enough for real-time commodity trading.' },
            ].map((card, i) => (
